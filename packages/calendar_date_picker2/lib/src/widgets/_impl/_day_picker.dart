...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final int dayOffset = getMonthFirstDayOffset(year, month,
        widget.config.firstDayOfWeek ?? localizations.firstDayOfWeekIndex);
    final int lastDayOffset = getMonthLastDayOffset(year, month,
        widget.config.firstDayOfWeek ?? localizations.firstDayOfWeekIndex);
    ...
    // a leap year.
    int day = -dayOffset;
    while (day < daysInMonth + lastDayOffset) {
      day++;
      if (day < 1 || day > daysInMonth) {
        ...
        if (rangeOver) {
          ...
          if (widget.config.overRangedDayTextStyle != null) {
            customDayTextStyle = widget.config.overRangedDayTextStyle;
          }
        }
