...
/// See also:
///
///   * [AppInteractiveViewer.builder], whose builder is of this type.
///   * [WidgetBuilder], which is similar, but takes no viewport.
typedef AppInteractiveViewerWidgetBuilder = Widget Function(
...
///
/// By default, AppInteractiveViewer clips its child using [Clip.hardEdge].
/// To prevent this behavior, consider setting [clipBehavior] to [Clip.none].
/// When [clipBehavior] is [Clip.none], AppInteractiveViewer may draw outside of
...
/// To prevent dead areas where AppInteractiveViewer does not receive gestures,
/// don't set [clipBehavior] or be sure that the AppInteractiveViewer widget is the
...
///     which includes the use of AppInteractiveViewer.
///   * The [flutter-go demo](https://github.com/justinmc/flutter-go), which includes robust positioning of an AppInteractiveViewer child
///     that works for all screen sizes and child sizes.
///   * The [Lazy Flutter Performance Session](https://www.youtube.com/watch?v=qax_nOpgz7E), which includes the use of an AppInteractiveViewer to
...
/// {@end-tool}
@immutable
class AppInteractiveViewer extends StatefulWidget {
  /// Create an AppAppInteractiveViewer.
  ///
  ...
  /// Creates an AppInteractiveViewer for a child that is created on demand.
  ///
  ...
  AppInteractiveViewer.builder({
    ...
    this.alignment,
    this.trackpadScrollCausesScale = false,
    required AppInteractiveViewerWidgetBuilder this.builder,
  })  : assert(minScale > 0),
        assert(interactionEndFrictionCoefficient > 0),
        assert(minScale.isFinite),
        assert(maxScale > 0),
  ...
  /// If set to [Clip.none], the child may extend beyond the size of the AppInteractiveViewer,
  /// but it will not receive gestures in these areas.
  /// Be sure that the AppInteractiveViewer is the desired size when using [Clip.none].
  ///
  ...
  ///
  /// Passed with the [AppInteractiveViewer.builder] constructor. Otherwise, the
  ...
  final AppInteractiveViewerWidgetBuilder? builder;

  /// The child [Widget] that is transformed by AppInteractiveViewer.
  ///
  /// If the [AppInteractiveViewer.builder] constructor is used, then this will be
  ...
  /// is often useful when a child should be bigger than the AppInteractiveViewer.
  ///
  ...
  ///
  /// {@template flutter.widgets.AppInteractiveViewer.onInteractionEnd}
  ...
  ///
  /// A [GestureDetector] wrapping the AppInteractiveViewer will not respond to
  ...
  /// listeners are notified. If the value is set, AppInteractiveViewer will update
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      AppInteractiveViewer.getNearestPointOnLine(
  ...
  @override
  State<AppInteractiveViewer> createState() => _AppInteractiveViewerState();
}

class _AppInteractiveViewerState extends State<AppInteractiveViewer>
  ...
  Rect get _boundaryRect {
    ...
    assert(
      ...
      "AppInteractiveViewer's child must have nonzero dimensions.",
    );
  ...
  @override
  void didUpdateWidget(AppInteractiveViewer oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.child != null) {
      child = _AppInteractiveViewerBuilt(
        childKey: _childKey,
      ...
      // When using AppInteractiveViewer.builder, then constrained is false and the
      ...
      child = LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          return _AppInteractiveViewerBuilt(
            childKey: _childKey,
            clipBehavior: widget.clipBehavior,
            constrained: widget.constrained,
            alignment: widget.alignment,
...
// AppInteractiveViewer's depending on if it's using a builder or a child.
class _AppInteractiveViewerBuilt extends StatelessWidget {
  const _AppInteractiveViewerBuilt({
    required this.child,
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return AppInteractiveViewer.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        AppInteractiveViewer.getNearestPointInside(point, boundary);
...
/// This enum is used to specify the behavior of the [AppInteractiveViewer] when
